...
fn main() -> std::io::Result<()> {
    ...
    println!("PDF: {} - Kind: {} (is_document: {})", pdf.mime(), pdf.kind(), pdf.kind().is_document());

    // ARCHIVE
    let zip_data = b"PK\x03\x04";
    let zip = detect(zip_data);
    println!("ZIP: {} - Kind: {} (is_archive: {})", zip.mime(), zip.kind(), zip.kind().is_archive());

    // TEXT
    let html_data = b"<!DOCTYPE html>";
    let html = detect(html_data);
    println!("HTML: {} - Kind: {} (is_text: {})", html.mime(), html.kind(), html.kind().is_text());

    // AUDIO
    let mp3_data = b"\xff\xfb\x90\x00";
    let mp3 = detect(mp3_data);
    println!("MP3: {} - Kind: {} (is_audio: {})", mp3.mime(), mp3.kind(), mp3.kind().is_audio());
    ...
    println!("MP4: {} - Kind: {} (is_video: {})", mp4.mime(), mp4.kind(), mp4.kind().is_video());

    // FONT
    let ttf_data = b"\x00\x01\x00\x00";
    let ttf = detect(ttf_data);
    println!("TTF: {} - Kind: {} (is_font: {})", ttf.mime(), ttf.kind(), ttf.kind().is_font());

    // EXECUTABLE
    let wasm_data = b"\x00asm";
    let wasm = detect(wasm_data);
    println!("WASM: {} - Kind: {} (is_executable: {})", wasm.mime(), wasm.kind(), wasm.kind().is_executable());

    // DATABASE
    let sqlite_data = b"SQLite format 3\x00";
    let sqlite = detect(sqlite_data);
    println!("SQLite: {} - Kind: {} (is_database: {})", sqlite.mime(), sqlite.kind(), sqlite.kind().is_database());
    ...
    println!("HTML contains TEXT: {}", html.kind().contains(MimeKind::TEXT));
